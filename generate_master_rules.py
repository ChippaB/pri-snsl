...
def main():
    ...
    for rule in sorted(rules, key=lambda x: x["part_id"]):
        if rule.get('is_consistent'):
            print(f"  {rule['part_id']:20} | {rule['count']} digits | {rule['prefix']:20} | {rule['total_records']:6,} records")
        else:
            print(f"  {rule['part_id']:20} | INCONSISTENT | {rule['total_records']:6,} records")
        )
